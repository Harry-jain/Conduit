...
class RecordingEngine:
    ...
    def __init__(self, sample_rate: int = 16000, out_dir: str = "data/enrollment") -> None:
    ...
    def record_sentence(self, sentence_text: str, sentence_index: int = 1, audio_np: np.ndarray | None = None) -> RecordingResult:
        ...
        mel = self.stft.audio_to_mel(np.interp(np.linspace(0, len(audio_np), int(duration_s * 22050), endpoint=False), np.arange(len(audio_np)), audio_np).astype(np.float32))
        wav_path = self.out_dir / f"segment_{sentence_index:03d}.wav"
        ...
        metadata = {"sentence_index": sentence_index, "sentence_text": sentence_text, "accepted": val.accepted}
