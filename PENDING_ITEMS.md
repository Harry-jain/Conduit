...
| P-02 | **Pending** | Production-grade cross-platform audio backends | §5 modules `audio/*`, `platform/windows/*`, `platform/macos/*`, §12 audio routing checks | `src/audio/*`, `src/platform/*` | WASAPI exclusive capture details, loopback flags, VB-Cable/BlackHole low-latency writer behavior, and platform-specific edge handling are not fully complete. |
| P-03 | **Pending** | Full PyQt enrollment UX contract | §5 `enrollment/ui.py`, §12 enrollment criteria | `src/enrollment/ui.py` | Full-screen sentence UX skeleton exists; waveform visualizer, word-highlighting progression, retry animation, and real DONE-triggered segment UI loop remain. |
...
| P-06 | **Pending** | OS-native overlay behavior parity | §5 `overlay/overlay_window.py`, §12 incoming caption criteria | `src/overlay/*`, `src/platform/windows/overlay_win.py`, `src/platform/macos/overlay_mac.py` | Current overlay still uses a generic PyQt path; exact native position/window-level behavior across all monitor/taskbar/dock combinations remains. |
| P-07 | **Partial** | Tray orchestration and self-healing process manager | §5 `tray/*`, §12 tray criteria | `src/tray/*` | Added service auto-restart monitor and dynamic status items (Outgoing/Incoming/GPU/Latency); still pending full language submenu and complete parity with specified tray menu contract. |
...
| P-09 | **Partial** | Colab workflow completeness | §5 `colab/notebook_generator.py`, §12 Colab criteria | `src/colab/*`, `colab/train_lora.ipynb` | Generator and launcher are implemented; still pending fully automated checkpoint retrieval/verification and stricter self-contained production notebook parity. |
